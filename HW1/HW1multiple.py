...
BA = np.transpose(np.zeros(numberRows))
gender = np.transpose(np.zeros(numberRows))
...
exog = np.transpose(np.vstack((BA, gender, age)))
exog = sm.regression.linear_model.add_constant(exog) # Needed for constant in regression equation

print(datetime.now()-startTime)

# Regression and Calculations
model = sm.regression.linear_model.OLS(AHE, exog)
results = model.fit()
